...
function loadAllExpenditureDetailsSubmitted() {
	$.ajax({
		"url" : "/academicactivity/loadAllExpenditureDetailsSubmittedForEdit",
		"method" : "GET",
		success : function(data) {
					 ...
					 /* 	$.ajax({
					 	...
					 	$.ajax({
					   		...
					   		success : function(data) {
								...
								console.log("remaininig paid"+remainingpaid);
						...
						$.ajax({
					 	...
					 	$.ajax({
					   		"url" : "/academicactivity/loadAllAmountPaidBackById",
					   		"method" : "GET",
					   		data:{"settlementId":settlementId},
					   		success : function(data) {
                              $('#paidBackGridBody').empty();
                   				...
                   				for(var i=0;i<data.length;i++)
								{
									  ...
									  var paidBackId=data[i]['universityPaymentBackId'];
									  console.log("paidId"+paidBackId);
				         		  ...
				         		  $.ajax({
									 ...
									 var sino=i+1;
									 console.log("crcodeeeeeeeeeee"+crCode);
									 tr=$('<tr class="paidback"  support="'+paidBackId+'" data-toggle="modal" data-target="#AmountPaidBackFormId"><td>'+sino+'</td><td>'+data[i]['paidBackDescription']+'</td><td>'+crCode+'</td><td>'+data[i]['voucharNo']+'</td><td>'+data[i]['paymentDate']+'</td><td>'+data[i]['amountPaidBack']+'</td><tr>');
									 $('#paidBackGridBody').append(tr);
									  $(".paidback").on('click',function(event) {   
										var paidId=  $(this).attr('support');
										
										$(".university").on('click',function(event) {  
   											var uniId=$(this).attr('support');
   										  $.ajax({
   										   		"url" : "/academicactivity/loadAllAmountPaidBackById",
   										   		"method" : "GET",
   										   		async:false,
...
/* 	  $('#expenditureDetailsForm').unbind().bind('submit',function(event) 
		  ...
		  var resourceId;
		  var vendorId;
		  alert("expenditureadd"+settlementId);
			    var formData = $(this).serializeArray();
			    console.log("sesettlementIddddddddddddddd"+settlementId);
							    ...
							    console.log("sumpaid="+sumPaid+"totalPaidbackuniversity="+totalPaidbackuniversity+"tobepaidactual="+tobepaidactual+"actualUnpaid="+actualUnpaid+"comingunpaid");
					   			...
					   			showMessage("total amount of expenditure unpaid should not be exceed  as granted amount");
						   			...
						   			/*if(sumPaid!=totalAdvanceSettled)
						   				...
						   				constrainError="could not execute statement; SQL [n/a]; constraint [uk_dv7ekuwotvvykbxcwxyqas0gl]; nested exception is org.hibernate.exception.ConstraintViolationException: could not execute statement";
							   			$.ajax({
					   						"url" : "/academicactivity/editSettlementRemarks",
					   						"method" : "POST",
					   						 data : formData,
					   						 success : function(data) {
	 ...
	 alert(justification+"   "+expId+"  "+recommended);
		 ...
		 $('#ProceduralcomplianceForm').unbind().bind('submit',function(event) 
			  	...
			  	$.ajax({
				    ...
				    var amnttobepaid=parseFloat(totalExpAmt)-amnttobepaid1;
					   console.log("amounttobepaid"+amnttobepaid+"totalExpAmt"+totalExpAmt+"amnttobepaid"+amnttobepaid1)
					    ...
					    $('#balAmntId').val(amnttobepaid);
					    $('#balAmountTobePaidId').val(balAmountTobePaid);
			   			setUpDataTable(data,[{"expId":"ExpenditureId"},{"billno":"Bill No/Vouchar No"},{"expamnt":"Value Amount"},{"taxAmnt":"Tax Amount"},{"advtotal":"Total Amount"},{"payee":"Payee Name"},{"gstno":"GST NO"},{"panno":"PAN NO"},{"paidStatus":"Status"},{"modepayment":"Mode of Payment"},{"headname":"Expenditure Head"}],"expendituregridTable","null");
			   			onDataTableClick('expendituregridTable', function() {		
							if(selectedRowFromDataTable != null)
							{
				...
				console.log("AdvanceViewAdvReqFormView");
					$.ajax({
			   	                 		  ...
			   	                 		  var totalPaidBack=0;
			   	                   				for(var i=0;i<data.length;i++)
			   									{
			   										 var univerId=data[i]['universityPaymentBackId'];
			   					         		  ...
			   					         		  $.ajax({
			   										...
			   										var sino=i+1;
			   										console.log("crcodeeeeeeeeeee"+crCode);
			   										 tr=$('<tr class="university" support="'+univerId+'" data-toggle="modal" data-target="#AmountPaidBackFormId1"><td>'+sino+'</td><td>'+data[i]['paidBackDescription']+'</td><td>'+crCode+'</td><td>'+data[i]['voucharNo']+'</td><td>'+data[i]['paymentDate']+'</td><td>'+data[i]['amountPaidBack']+'</td><tr>');
			   										 $('#paidBackGridBody').append(tr);
			   										$(".university").on('click',function(event) {  
			   											var uniId=$(this).attr('support');
			   										  ...
			   										  $.ajax({
			   										   		...
			   										   		data:{"paidId":uniId},
			   										   		success : function(data1) {
			   										   		 $('#universityPaymentBackId1').val(uniId);
			   												 $('#paidBackDescriptionId1').val( data1[0]['paidBackDescription']);
			   												 $('#costCenterCodeId1').val(data1[0]['costCenterCode']);
			   												 $('#voucharNoId1').val(data1[0]['voucharNo']);
			   												 $('#paymentDateId1').val(data1[0]['paymentDate']);
			   												 
			   												$('#amountPaidBackId1').val(data1[0]['amountPaidBack']);
			   							
			   														 	 $('#enteredRemarkspaidId1').val(data1[0]['enteredRemarks']);
			...
			$('#payeeNameId1').on('change', function() {
			 ...
			 $('#AmountPaidBackForm1').unbind().bind('submit',function(event) 
					   {
					   	        console.log("entrrr in AmountPaidBackForm");  
					   			event.preventDefault();
					   			var formCheck = "success";
								
								var uniPaidId=$('#universityPaymentBackId1').val();
					   			var formData = $(this).serializeArray();
					   			formData.push({"name":"ac","value":activityId});
					   			formData.push({"name":"setId","value":settlementId});
							//	formData.push({"name":"universityPaymentBackId","value":uniPaidId});
					   			
					   			constrainError="could not execute statement; SQL [n/a]; constraint [uk_dv7ekuwotvvykbxcwxyqas0gl]; nested exception is org.hibernate.exception.ConstraintViolationException: could not execute statement";
					   			
					   			$.ajax({
			   						"url" : "/academicactivity/editAmountPaidBacktoUniversityDetails",
			   						"method" : "POST",
			   						 data : formData,
			   						 success : function(data) {
			   							...
			   							console.log("data after getting saveAmountPaidBacktoUniversityDetails"+ data);
			   							 if(data==constrainError)
			   								 ...
			   								 {
			   								showMessage(data);
			   								$("#AmountPaidBackForm1").trigger('reset');
			   								 }
			   						//	advanceSettlementId=data['setId'];
...
<div class="card-header">
   <h3>Edit for Expenditure Sanction / Settlement of Advance - Details of Bills , Vouchers and utilisation of Advance </h3>
</div>
<div class="card-body">
<div class="row">
...
<div id="tabs">
	...
	<li style="background-color:#5a2066;text-color:white;"><a href="#tabs-2" style="color:#d5e6e4">Remarks</a></li>
	 </ul>
	   ...
	   <div id="advanceRecieveTabId">
	     ...
	     <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#AmountPaidBackFormId">
            ...
            <div class="row">
	       <p>
	       <button type="button" class="btn btn-primary" data-toggle="collapse" data-target="#amountPaidBackView" id="amntpaidbackId">
         ...
         <div class="modal-body">
            <form id="AmountPaidBackForm">
               <div class="row">
                  <div class=" col-lg-12 col-md-12 col-sm-12 col-xs-12">
                   <!--  <div class="form-group">
                        <label for="paidbackiId">University PaidBackId</label> <input type="text"
                           name="paidBackDescription" class="form-control" id="universityPaymentBackId" required />
                     </div> -->
                     <div class="form-group">
...
<!--  university paid back information -->

  <!-- Details of any Amount Paid back to the University Details -->
	     <!-- Modal HTML End -->
<!-- Modal Resource Person-->
<div class="modal fade" id="AmountPaidBackFormId1" tabindex="-1"
   role="dialog" aria-labelledby="exampleModalLongTitle"
   aria-hidden="true">
   <div class="modal-dialog" role="document">
      <div class="modal-content">
         ...
         <div class="modal-body">
            <form id="AmountPaidBackForm1">
               <div class="row">
                  <div class=" col-lg-12 col-md-12 col-sm-12 col-xs-12">
               <div class="form-group">
                        <label for="paidbackiId">University PaidBackId</label> <input type="text"
                           name="universityPaymentBackId" class="form-control" id="universityPaymentBackId1" required />
                     </div> 
                   <div class="form-group">
                        <label for="paidBackDescriptionId">Description</label> <input type="text"
                           name="paidBackDescription" class="form-control" id="paidBackDescriptionId1" required />
                     </div>
                     <div class="form-group" style="width:100%">
						<label for="centerName">Cost Center Code<span class="required">*</span></label>
						 		<div style="width:100%">
							 		<div>
									<select class="form-control selectpicker"  name="costCenterCode" id="costCenterCodeId1" required>																		  										  
                     ...
                     <div class="form-group">
                        <label for="paymentDateId"> Vouchar Date</label> <input type="text"
                           name="paymentDate" class="form-control dp" id="paymentDateId1"  />
                     </div>
                     <div class="form-group">
                        <label for="activitytype"> Amount</label> <input type="number"
                           name="amountPaidBack" class="form-control" id="amountPaidBackId1"  />
                     </div>
                     <div class="form-group">
                        <label for="activitytype"> Remarks</label> <input type="text"
                           name="enteredRemarks" class="form-control" id="enteredRemarkspaidId1"  />
                     </div>
                  </div>
                  <div class="modal-footer">
                     <button type="button" class="btn btn-secondary"
