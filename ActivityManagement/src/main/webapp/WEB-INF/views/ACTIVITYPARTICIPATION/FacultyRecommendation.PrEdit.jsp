...
alert("gettotalexpense");
	   ...
	   //  alert(sum);
	     ...
	     $.ajax({
					...
					success:function(data)
					{
						...
						for(var i=0;i<data.length;i++)
							{
							onWardJourneyAmountEstim=onWardJourneyAmountEstim+data[i]['on_ward_est_expenditure'];
							returnJourneyAmountAdmit=returnJourneyAmountAdmit+data[i]['return_est_expenditure'];
							accomAmountAdmit=returnJourneyAmountAdmit+data[i]['accmdtion_estim_expenditure'];
							}
		   ...
		   var total= sum1+onWardJourneyAmountEstim+accomAmountEstim+accomAmountEstim;
  ...
  function getTotalExpense()
  {
	...
	function loadAllparticipationRequestRecommendedBy() {
		$.ajax({
			"url" : "/academicactivity/getAllParticipantRequestRecommendedBy",
			"method" : "GET",
			success : function(data) {
...
function loadParticipantRequestApproval() {
	...
	$.ajax({
		/* "url" : "/academicactivity/getAllParticipantRequestForFacultyRecom", */
		"url" : "/academicactivity/getAllParticipantRequestRecommendedBy",
		"method" : "GET",
		success : function(data) {
				...
				if(selectedRowFromDataTable != null)
				{
					...
					$.ajax({
									...
									success : function(data) 
									{
										...
										$('#recommStatusId').val(data['participationRequestStatus']);
...
$(document).ready(function() {
	...
	loadAllparticipationRequestRecommendedBy();
	$.ajax({
	   ...
	   $('#onwardJourneyDetails').unbind().bind('submit',function(event) 
			   {
			   			...
			   			//	$('#confId').unbind().bind('click',function(){
			   	             ...
			   	             formData.push({"name":"onwardAmountAdmitted","value":0})
			   					...
			   					$.ajax({
			   						...
			   						"method" : "POST",
			   						 ...
			   						 success : function(data) {
			   							...
			   							onWardJourneyAmountEstim=parseFloat($('#onWardEstExpenditureId').val())+onWardJourneyAmountEstim;
   		...
   		$('#otherExpenseForm').unbind().bind('submit',function(event) {
	                    ...
	                    var estim=$('#totalExpenseProgramId').val();
	                    formData.push({"name":"participationRequestId","value":particiaptionRequestId});
	                   // formData.push({"name":"totalExpenseProgram","value":estim});
	...
	$('#returnJourneyDetails').unbind().bind('submit',function(event) 
			   {
			   			...
			   			//	$('#confId').unbind().bind('click',function(){
			   	         ...
			   	         formData.push({"name":"returnAmountAdmitted","value":0});
			                     ...
			                     var i=0;
			   					$.ajax({
			   						...
			   						"method" : "POST",
			   						 data : formData,
			   						 success : function(data) {
			   							returnJourneyAmountEstim=parseFloat($('#returnEstExpenditureId').val())+returnJourneyAmountEstim;
			   							 console.log(formData);
			   							 showMessage(data);
			   							loadParticipantRequestApproval();
			   							getPersonalDetails(participateRequestId)
	...
	$('#AccomadationDetails').unbind().bind('submit',function(event) 
			   {
			   			...
			   			//	$('#confId').unbind().bind('click',function(){
			   	            formData.push({"name":"participateRequestId","value":participateRequestId});
			   	            formData.push({"name":"activityCode","value":activityId});
			   	            formData.push({"name":"amountAdmitted","value":0});
			   					...
			   					$.ajax({
			   						...
			   						"method" : "POST",
			   						 ...
			   						 success : function(data) {
			   							...
			   							loadParticipantRequestApproval();
			   							getPersonalDetails(participateRequestId)
			   							accomAmountEstim=parseFloat($('#estimExpId').val())+accomAmountEstim;
			   							//particiaptionRequestId=data['particiaptionRequestId'];
			   							//showMessage("Parcipation Request is successfully added"+particiaptionRequestId);
	...
	$('#facultyRecommendation').unbind().bind('submit',function(event) {
        event.preventDefault();
        var formData = $(this).serializeArray();
		...
		$('#confId').unbind().bind('click',function(){
			console.log("confirm clicked...");
			$.ajax({
				"url" : "/academicactivity/addFacultyRecommedationDetails",
				"method" : "POST",
				 data : formData,
				 success : function(data)
				 {
...
<div class="card-header">
   <h3>Edit Participation Request Faculty Recommendation Edit</h3>
</div>
<div class="card-body">
<div class="row">
...
<div id="tabs">
	...
	<li style="background-color:#3d5e48;text-color:white;"><a href="#tabs-3" style="color:#d5e6e4">Other Expenses</a></li>
  	...
  	<li style="background-color:#d92176;text-color:white;"><a href="#tabs-5" style="color:#d5e6e4">Recommendation</a></li>
	  </ul>
	  <div id="tabs-1">
...
<tbody>
   ...
   <td><input type="submit" value="Seen And Verify"/></td>
    ...
    <tr>
      ...
      <td></td>
      <td></td>
      <td><input type="submit" value="Add" value="Seen And Verify"/></td>
      </tr>
	 </table>
	 </div>          
	   </div>
	...
	<div class="row">
	   <div id="otherDetailsId">
	   <form id="facultyRecommendation">
	   <div class="row">
	    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
            <div class="form-group">
               <label for="remarks"> Remarks</label>
               <textarea id="enteredRemarksId" name="enteredRemarks" rows="4" cols="100"
	      ...
	      <div class="form-group" class="col-lg-6 col-md-6 col-sm-6 col-xs-6">
               ...
               <div style="width: 100%">
                  <div style="display: inline-block; width: 100%">
                     <select class="form-control selectpicker" name="recomStatus"
                        id="recommStatusId" required>
                        <option value="se">Select</option>
                        <option value="recommended">Recommend</option>
                        <option value="invalidate">Reject</option>
          ...
          <!--   <div class="form-group">
               <label>Recommended Date</label> <input type="text" id="approvedDate"
                  name="recomDate" class="form-control dp" required></input>
            </div> -->
...
<div  id="gridRow" class="card card-info card-outline">
  	...
  	<div class="card-header">
		  <h3 >List Of Program Participation Recommended</h3>
