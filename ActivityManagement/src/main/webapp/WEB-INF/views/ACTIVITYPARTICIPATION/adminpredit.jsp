						...
						for(var i=0;i<data.length;i++)
					     ...
					     $.ajax({
								...
								setUpDataTable(data1, [ {
									...
									{
										"amount_admitted":"Onward Admitted Expenditure"
									}
								...
								onDataTableClick('participantAccomadationTable',function()
								  ...
								  setUpDataTable(data1, [ {
									...
									{
										"return_amount_admitted":"Return Amount Admitted"
									}
	...
	//alert("advance clicked");
	  ...
	  $.ajax({
					...
					success:function(data)
					{
						...
						for(var i=0;i<data.length;i++)
							{
							onWardJourneyAmountAdmit=onWardJourneyAmountAdmit+parseFloat(data[i]['onward_amount_admitted']);
							returnJourneyAmountAdmit=returnJourneyAmountAdmit+parseFloat(data[i]['return_amount_admitted']);
							accomAmountAdmit=accomAmountAdmit+parseFloat(data[i]['amount_admitted']);
							}
					
							
						alert("daaaaaaaaaaata"+"onWardJourneyAmountAdmit="+onWardJourneyAmountAdmit+"returnJourneyAmountAdmit="+returnJourneyAmountAdmit+"accomAmountAdmit="+accomAmountAdmit);	
					},
					error(e)
					{
						console.log("Error while request"+e)
	  ...
	  admitTotal=0;
	  admitTotal=sum2+onWardJourneyAmountAdmit+returnJourneyAmountAdmit+accomAmountAdmit;
	   
	 //  alert("onwardJourneyadmit"+onWardJourneyAmountAdmit);
	   //alert("returnJourneyAmountAdmit"+returnJourneyAmountAdmit);
	  // alert("accomAmountAdmit"+accomAmountAdmit);
	   
	  // alert("admittotal"+admitTotal);

	   
	 //  alert("sadmitTotalum2="+admitTotal);
	   //$('#totalExpenseProgramId').val(total);
	   $('#totalAdmitAmountId').val(admitTotal);
...
function loadAllparticipationRequestAdministrativeBy() {
	$.ajax({
		"url" : "/academicactivity/getAllParticipantAdministrativeApprovedBy",
		"method" : "GET",
		success : function(data) {
...
function loadParticipantRequestApproval() {
	...
	$.ajax({
		"url" : "/academicactivity/getAllParticipantAdministrativeApprovedByStatus",
		"method" : "GET",
		success : function(data) {
					...
					$.ajax({
									...
									success : function(data) 
									{
                                        ...
                                        $("#totalAdmitAmountSanctionId").show();
                                     	$('#adminstrativeSanctionId').val(data['participationRequestStatus']);
										var teamMember=data['teamParticipation'];
										//console.log(financeImplied);
										//var financeImplied=data['ac']['activityCode']
									//	if(data['participationRequestStatus']=="adminstrativeapproved")
	 				                    //     {
	 			                    	$('#totalAdmitAmountSanctionId').show();
	 				//   getTotalExpense();
	 				
	 				              //      var admittcalculated=$('#totalAdmitAmountId').val();
	 				                    
	 				                    console.log("totalAdmitAmountSanctionId"+admittcalculated);
	 				            //        $('#totalAdmitAmountSanctionId').val(data['totalAdmitAmount']);
	 				                   $('#totalAdmitAmountId').val(data['totalAdmitAmount']);
	 				                    
	 				                   

                                      //   }
	 			                        // if(value=="adminstrativerejected")
 				                        //   {
						...
						success:function(data)
						{
							...
							var sumestim=0;
							   $('.estim').each(function()
								   		{
								    var amnt66=$(this).val();
								    sumestim=sumestim+parseFloat(amnt66 ); 
							     
								   		});
							   $('#otherEstimeExpenseTotalId').val(sumestim);
...
$(document).ready(function() {
	...
	loadAllparticipationRequestAdministrativeBy();
...
$('#viewaccombt').unbind().bind('click',function(){
	   					...
	   					$.ajax({
	   						...
	   						"method" : "POST",
	   						 ...
	   						 success : function(data) {
	   							...
	   							showMessage("Parcipation Request is successfully verified"+particiaptionRequestId);
	   							loadParticipantRequestApproval();
	...
	$('#adminstrativeSanctionId').change(function()
	 		{
	 			var value=$(this).val();
	 			
	 			if(value=="adminstrativeapproved")
	 				{
	 				$('#totalAdmitAmountSanctionId').show();
	 				//   getTotalExpense();
	 			//	$('#totalAdmitAmountSanctionId').val(admitTotal);
	   ...
	   $('#onwardJourneyDetails').unbind().bind('submit',function(event) 
			   {
			   	      ...
			   	      formData.push({"name":"onwardAmountClaimed","value":0});
			   					...
			   					$.ajax({
			   						...
			   						"method" : "POST",
			   						 ...
			   						 success : function(data) {
			   							console.log(formData);
			   							showMessage(data);
			   							onWardJourneyAmountAdmit=parseFloat($('#onwardAmountAdmittedId').val())+onWardJourneyAmountAdmit;
			   							loadParticipantRequestApproval();
			   							getPersonalDetails(participateRequestId);
			   							//particiaptionRequestId=data['particiaptionRequestId'];
	...
	$('#returnJourneyDetails').unbind().bind('submit',function(event) 
			   {
			   			...
			   			//	$('#confId').unbind().bind('click',function(){
			   	            formData.push({"name":"participateRequestId","value":participateRequestId});
			   	         formData.push({"name":"activityCode","value":activityId});
			   	      formData.push({"name":"returnAmountClaimed","value":0});
			   					...
			   					$.ajax({
			   						...
			   						"method" : "POST",
			   						 ...
			   						 success : function(data) {
			   							...
			   							loadParticipantRequestApproval();
			   							getPersonalDetails(participateRequestId);
			   							returnJourneyAmountAdmit=parseFloat($('#returnAmntAdmtId').val())+returnJourneyAmountAdmit;
			   							//particiaptionRequestId=data['particiaptionRequestId'];
			   							//showMessage("Parcipation Request is successfully added"+particiaptionRequestId);
	...
	$('#AccomadationDetails').unbind().bind('submit',function(event) 
			   {
			   			...
			   			//	$('#confId').unbind().bind('click',function(){
			   	            formData.push({"name":"participateRequestId","value":participateRequestId});
			   	            formData.push({"name":"activityCode","value":activityId});
			   	         formData.push({"name":"amountClaimed","value":0});
			   					console.log("confirm clicked...");
			                     var i=0;
			   					$.ajax({
			   						...
			   						"method" : "POST",
			   						 ...
			   						 success : function(data) {
			   							...
			   							loadParticipantRequestApproval();
			   							getPersonalDetails(participateRequestId);
			   							accomAmountAdmit=parseFloat($('#amntAdmtId').val())+accomAmountAdmit;
			   							//particiaptionRequestId=data['particiaptionRequestId'];
			   							//showMessage("Parcipation Request is successfully added"+particiaptionRequestId);
	...
	$('#administrativeapproval').unbind().bind('submit',function(event) {
		...
		$('#confId').unbind().bind('click',function(){
			console.log("confirm clicked...");
			$.ajax({
				"url" : "/academicactivity/addAdminstrativeDetails",
				"method" : "POST",
				 data : formData,
				 success : function(data)
				 {
...
<div class="card-header">
   <h3>Edit For Administrative Approval for Participation Request </h3>
</div>
<div class="card-body">
<div class="row">
...
<div id="tabs">
	...
	<li style="background-color:#3d5e48;text-color:white;"><a href="#tabs-3" style="color:#d5e6e4">Other Expenses</a></li>
  	...
  	<li style="background-color:#d92176;text-color:white;"><a href="#tabs-5" style="color:#d5e6e4">Administrative Approval
  	</a></li>
	  </ul>
	  <div id="tabs-1">
 ...
 <div id="otherExpenseId">
	   <div class="row">
	    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
	         ...
	         <p>
          <button class="btn btn-link" type="button" data-toggle="collapse" data-target="#memberPersonal" aria-expanded="false" aria-controls="collapseExample" id="viewperbt">
                  View Member Person Details
          </button>
            
             <button class="btn btn-link" type="button"
              data-toggle="collapse" data-target="#memberOnwardDetails" aria-expanded="false" aria-controls="collapseExample" id="viewonwardbt">View OnwardTravel Details
            </button>
             <button class="btn btn-link" type="button"
              data-toggle="collapse" data-target="#memberReturnDetails" aria-expanded="false" aria-controls="collapseExample" id="viewreturnbt">View Return Travel Details
            </button>
            <button class="btn btn-link" type="button"
            data-toggle="collapse" data-target="#accomadationDetails" aria-expanded="false" aria-controls="collapseExample" id="viewaccombt">View Accomadation Details
            </button>
         </p>
									...
									<div class="collapse" id="memberOnwardDetails">
										<div id="gridRow" class="card card-info card-outline">
											<div class="card-header">
												...
												<div class="table-responsive">
													<table class="table" id="participantOnwardTable"></table>
												</div>
	   ...
	   <div class="row">
	    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
            <div class="form-group">
               ...
               <div style="width: 100%">
                  <div style="display: inline-block; width: 100%">
                     <select class="form-control selectpicker" name="adminstrativeStatus"
                        id="adminstrativeSanctionId" required>
                        <option value="se">Select</option>
                        <option value="adminstrativeapproved">Approve</option>
                        <option value="adminstrativerejected">Reject</option>
            ...
            <div class="form-group">
               <label for="remarks"> Administrative Sanction Amount</label>
               <input type="number" name="totalAdmitAmount" id="totalAdmitAmountSanctionId"/>
            </div>
          ...
          <!--   <div class="form-group">
               <label>Approve Date</label> <input type="text" id="adminApprovedDate"
                  name="adminApprovedDate" class="form-control dp" required></input>
            </div> -->
...
<div  id="gridRow" class="card card-info card-outline">
  	...
  	<div class="card-header">
		  <h3 >List Of Program Administrative Verified</h3>
