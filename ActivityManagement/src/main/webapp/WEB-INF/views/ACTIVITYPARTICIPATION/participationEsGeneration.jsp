...
function loadParticipantRequestApproval() {
	...
	$.ajax({
		"url" : "/academicactivity/getAllParticipationSettlementDetailsforEs",
		"method" : "GET",
		success : function(data) {
			...
			onDataTableClick('participantActivityTable', function() {
				...
				if(selectedRowFromDataTable != null)
				{
					...
					$.ajax({
		        		"url" : "/academicactivity/getActivityBudHeadDetailsbyParticipation",
		        		"method" : "GET",
		        		data :{"pr":particiaptionRequestId},
		        		success : function(data) {
		        			console.log(data);
                  ...
                  $.ajax({
						...
						success:function(data)
						{
var totalExpense=data['totalExpenseProgram'];
							
							$('#EstExpId').val(data['totalAdmitAmount']);
							$('#sanctionedAmountId').val(data['totalClaimedAmount']);
...
function getAllEsOrderGenerationInformation() {
	console.log("getAllEsOrderGenerationInformation");
	$.ajax({
		"url" : "/academicactivity/getAllEsOrderGenerationInformation",
		"method" : "GET",
		success : function(data) {
			...
			{
				"participateRequestid":"Participate Requuest Id"	
			},
...
$(document).ready(function()
	...
	getAllEsOrderGenerationInformation();
/*	$.ajax({
...
// 		     	formData.push({"name":"participateRequestid","value":particiaptionRequestId});
				...
				$('#confId').unbind().bind('click',function(){
					...
					$.ajax({
						"url" : "/academicactivity/participationEsOrderGeneration",
						"method" : "POST",
						 data : formData,
						 success : function(data) {
							console.log(data);
...
$('#submitModel').modal('hide');
	 ...
	 $('#budHead').on('change', function() {
		//  alert("budhead details");
			...
			$.ajax({
 			"url" : "/academicactivity/getBudgetHeadAmountForParticipation",
					"method" : "GET",
					 data : {"budheadId":budheadId,"finyear":finyear,"pr":particiaptionRequestId},
					 success : function(data) {
			   						...
			   						"method" : "POST",
										...
										var data_splits = data.split("-");
			   							if(data_splits[0] == "SAVED")
			   							{
			   								...
			   								budstring=budstring+headdname+"-  "+amountTobemapped;
			   								
			   								
			   								console.log(headdname);
				   							if(!map[headdname])//if(typeof map[headdname]=='undefined')
				   							{
				   						         map[headdname]={};
				   							}
				   							else
				   							{
				   								map[headdname]=amountTobemapped;
			   								}
			   							     console.log(map);
			   							}
			   								else
	...
	$('#printBt').unbind().bind('click',function()
	   ...
	   $.ajax({
                "url" : "/academicactivity/getEsOrderGenerationParticipation",
                "method" : "GET",
	   ...
	   $.ajax({
			 ...
			 success:function(data)
				...
				$('#estimtab').append(tr);
				 ...
				 var orderRemaining="The expenditure of Rs.     "+planfund+"may be met form the Head of"+budgetInvolved+"Expenditure (02) Academic programmes and other activities for the Financial Year"+finyear;
...
<div id="gridRow" class="card card-info card-outline">
	<div class="card-header">
		<h3>List Of Program Participation For Es Generation</h3>
...
<tr><td id="esNumberPrefix"></td><td style="text-align:right" id="orderGeneratedDate"></td></tr>
<tr><td id="esrefer"></td></tr>
<tr><td colspan="2" style="text-align:center"><u><b>ORDER</b></u></td></tr>
<tr><td colspan="2" style="text-align:justify" id="esOrderContent"></td></tr>
...
<tr><td>Copy to</td><td style="text-align:right">By order of the Vice-Chancellor</td></tr>
