...
var totaldeviationIdRep;
  ...
  var resources=new Array();
   ...
   //for nuals allocation amount;
   var totalExpHeadAmount=0;
  ...
  var hl=0;
   ...
   function loadAllActivitySubmitted() {
		console.log("loadAllActivitySubmitted");
		$.ajax({
			"url" : "/academicactivity/getAllActivityForStudentSubmitted",
			"method" : "GET",
			success : function(data) {
			  ...
			  ], "submittedactivityTable", "select-checkbox");
			},
			error()
			{
...
function getTotalEstimatedExpenditure()
{
   ...
   function getAllExpHeadDetails()
   {
	   ...
	   function updateActivityMasterAsno()
	   {
		  //    var asNoFormData=new Array();
		    //  var asMap=new Map();
		    //  asMap['activityCode']=activityId;
		    //  asNoFormData.push(asMap);
		      
		     // var asprefix="NUALS/ACCD/CPGIS/AC2/52/2016";
		      $.ajax({
				   	"url":"/academicactivity/GetActivityMaster",
				   	"method":"GET",
					 data:{"activityId":activityId},
				   	  success:function(data)
				   	 {
				   		console.log("count of asno"+data);
   ...
   */
   ...
   $(document).ready(function()
   {
   	...
   	$("#submitDate").datepicker().datepicker("setDate", new Date());
   	
   	//to display financial year
...
/*  	//to get all activitytypes
 	  $('#ActivityTypeGroupId').change(function(){ 
 		  alert("catgry selected");
       var cat = $(this).val();
 	     $.ajax({
      		"url" : "/academicactivity/getAllActivityTypesByCategory",
      		"method" : "GET",
   	...
   	//to get the faculty co-ordinator
   		...
   		success : function(data) {
   			...
   			setUpDropDown("cordinatorId",data,"emp_id","emp_name");
   		  	...
   		  	$.ajax({
   				"url":"/academicactivity/getUserInfo",
   				"method":"GET",
   				success:function(userdata)
   					{
   					console.log(userdata);
   					
   					$('#cordinatorId').val(userdata['emp_id']);
   	...
   	formData.push({"name":"financeImplied","value":financeImplied});
   			...
   			var formCheck = "success";
   				...
   				$('#confId').unbind().bind('click',function(){
   					...
   					$.ajax({
   						"url" : "/academicactivity/addActivityMasterByStudent",
   						"method" : "POST",
   						 data : formData,
   						 success : function(data) {
   							console.log(data);
   							//alert(data);
   								...
   								if(data=="Activity Name is already Exist")
   									...
   									centreDialog("Success","Program Details are Successfully added with Program Id."+res[1],"green");
   ...
   $("#resourcePersonviewbtId" ).on('click',function(event) {
   	var length=resourcepers.length;
   	console.log("resource person length"+length);
   	if(length>0)
   		{
   	$.ajax(
   			{
   		...
   		data:{"activityId":activityId},
   		 success:function(data)
   		 {
   			try
   			{
   				...
   				var fundallocatedId	;
   				var expincuId;
   				var Balance_exclude_unassignedGrant;
   				var estimatedExp;
   				var natnlsemnrcount;
   				var lecturecount;
   					...
   					for(var i=0;i<data.length;i++)
   					{
   						...
   						var currentfinyear=$("#finYearId").val();
   						centerId=data[i]["actvtyCenterKey"]["cm"]["centre_id"];	
   						centerarray[i]=centerId;
   					}
   					$.ajax({
							"url":"/centerfund/getAllCenterFundDetailsByCenterIdsAndFinYear",
							"method":"GET",
							  data:{"currentfinyear":currentfinyear,"centerarray":centerarray.toString()},
							 success:function(data1)
							 {
							console.log(data1.length);		
							for(var i=0;i<data1.length;i++)
   					         {
							 var centerId1=data1[i]["centreFundKey"]["cm"]["centre_id"];
							 var nationalSeminarCount=0;
							 var lecount=0;
							$.ajax({
									"url":"/academicactivity/getActivityCountBasedOnactivitytype",
									"method":"GET",
									  data:{"finYear":currentfinyear,"centreId":centerId1},
									  async:false,
									 success:function(seminarcount)
									 {
										 
										 var secount=seminarcount.length;
										 
										 if(secount>0)
											 {
											 nationalSeminarCount=secount;
											 }
										 else
											 {
											 nationalSeminarCount=0;
											 }
										 
											$.ajax({
												"url":"/academicactivity/getActivityCountBasedOnactivitytypeLecture",
												"method":"GET",
												  data:{"finYear":currentfinyear,"centreId":centerId1},
												  async:false,
												 success:function(lecturecount)
										           {
													 var lecount1=lecturecount.length;
							 ...
							 console.log(nationalSeminarCount);
							 
							 console.log(data1[i]["centreFundKey"]["cm"]["centre_id"]);
							...
							var unassignedGrantAmnt=data1[i]["unAssignGrantBal"];
							var commitTotal=data[i]['expTillDate'];
							balHeadArray[i]=bal;
							var tr=$('<tr><td>'+centername+'</td><td><input class="centerHeads form-control" id="'+fundallocatedId+'" type="number" name="'+fundallocatedId+'" value="'+allocatedfundamount+'"  disabled="disabled" width="125%"></input></td><td><input class="exptill form-control" id="'+expincuId+'" type="number" value="'+exptilldate+'" name="'+expincuId+'"  disabled="disabled" width="125%"></input></td><td><input class="balassign form-control" id="'+Balance_exclude_unassignedGrant+'" type="number" name="'+Balance_exclude_unassignedGrant+'" value="'+bal+'"  disabled="disabled" width="125%"></input></td> <td><input class="estimateHeads form-control" id="'+estimatedExp+'" type="number" name="'+estimatedExp+'" value="0" width="125%"></input></td> <td><input class="seminarHeads form-control" id="'+natnlsemnrcount+'" type="number" name="'+natnlsemnrcount+'" value="'+nationalSeminarCount+'"></input></td><td><input class="lectureHeads form-control" id="'+lecturecount+'" type="number" name="'+lecturecount+'" value="'+lecount+'"></input></td><td><input class="unassigrant form-control" id="'+unassignGrant+'" type="number" name="'+unassignGrant+'" value="'+unassignedGrantAmnt+'"></input></td><tr>');
						      ...
						      error:function(e)
					         {
						         showMessage("no center fund for particular center. Contact Admin");
				            }
					    });
   					...
   					{
   					showMessage("no centers are in this activity ");
   					$('#centerActivityeBody2').empty();
   					}
   			}
   			catch(e)
   ...
   //to get the unassignedgrant amnt
   				...
   				console.log("unAssignGrantAmnt"+unAssignGrantAmnt);
   					...
   					//	des=removeWhiteSpaceCharacter(des);
   						...
   						console.log("nualsalloctinamountValue"+nualsalloctinamountValue);
   						 ...
   						 tr=$('<tr><td>'+sino+'</td><td>'+data[i]["expHeadsKey"]["description"]+'</td><td><input class="allocationheads form-control" id="'+nualsalloctinamount+'" type="number"  value="0"></input></td><td><input class="estimateheads1 form-control" id="'+estmtdexpdture+'" type="number"  value="0"></input></td><td><input class="deviationheads form-control" id="'+deviation+'" type="number"  value="0"></input></td><td><input class="commentsheads form-control" id="'+comments+'"  ></input></td><tr>');
   					...
   					$('#advnceRequestGridBody').empty();
   						...
   						for(var i=0;i<data.length;i++)
   						{
   							...
   							var sino=i+1;
   							 ...
   							 tr=$('<tr><td>'+sino+'</td><td>'+data[i]["expHeadsKey"]["description"]+'</td><td><input class="expadvreq form-control" id="'+advanceRequired+'" type="number" value="0"></input></td><td><input class="expadvreqreason form-control" id="'+requirementReason+'"  ></input></td><tr>');
       			...
       			try
       		{
       				...
       				$('#centerActivityeBody3').empty();
       					...
       					for(var i=0;i<data.length;i++)
       					{
       						...
       						var externalfundamnt=data[i]["actvtyCenterKey"]["cm"]["centre_id"]+"externalfund"
                    ...
                    //     var tr=$('<tr><td>'+data[i]["actvtyCenterKey"]["cm"]["centre_name"]+'</td><td><input class="centerHeads form-control" id="'+externalFund+'" type="number" name="externalFund"></input></td></tr>');
       						var tr=$('<tr><td>'+data[i]["actvtyCenterKey"]["cm"]["centre_name"]+'</td><td><input class="externlfunds form-control" id="'+externalfundamnt+'" type="number" name="externalFund" value="0"></input></td></tr>');
       		 ...
       		 success:function(data)
       		 {
       			try
       				...
       				$('#centerActivityeBody4').empty();
       					...
       					for(var i=0;i<data.length;i++)
       					{
       						var ugcfund=data[i]["actvtyCenterKey"]["cm"]["centre_id"]+"externalfund"
       						//alert(data[i]);
       						console.log(data[i]["actvtyCenterKey"]["cm"]["centre_name"]);
       						var ugcFundAmnt=data[i]["actvtyCenterKey"]["cm"]["centre_id"]+"ugcFund";
       					
                               var tr=$('<tr><td>'+data[i]["actvtyCenterKey"]["cm"]["centre_name"]+'</td><td><input class="ugcFunds form-control" id="'+ugcFundAmnt+'" type="number" name="ugcFund" value="0"></input></td></tr>');
   ...
   $('#privateFund').change(function(){ 
       var value = $(this).val();
       if(value=="yes")
       	{
    	   $('#privatefundingjustification').hide();
       	}
	   ...
	   for(var i=0;i<totalHeadLength;i++)
		   {
               ...
               console.log("valueeeeeeeeeee"+comment);
   ...
   $.ajax({
   	...
   	"method":"GET",
   	 ...
   	 success:function(data)
   	 {
   	   ...
   	   $.ajax({
   	   	"url":"/centerfund/updateActivityCenterFund",
   	   	"method":"GET",
   	   	 data:{"activityId":activityId,"centerarray":centerarray.toString(),"finyear":finYear},
   	   	 success:function(data)
   	   	 {
...
/*if(privateFundValue=="se")
	{
	showMessage("Please select Private fund value")
	}*/
		...
		formData2.push({"name":"ac","value":activityId});
	     ...
	     $.ajax({
		   	"url":"/academicfinance/addActivityFinance",
		   	"method":"POST",
		   	"data":formData2,
		    success:function(data)
		   	 {
		   		...
		   		fileFormData.append('activityId',activityId);
					$.ajax(
					  {
						"url" : "/academicactivity_public/saveFinanceDocs",
						"method" : "POST",						
						data : fileFormData,
						processData: false, // Don't process the files
						contentType: false, 
   ...
   $.ajax({
	   	"url":"/expenditurehead/addActivityEstimate",
	   	"method":"GET",
   ...
   //add advance estimate request ajax call
   	...
   	var map={};
   				    ...
   				    formData1.push(map);
      $.ajax({
	   	"url":"/expenditurehead/addActivityAdvanceEstimateRequest",
	   	"method":"POST",
	   	data:{"activtyEstimateAdvncewrapper":formData1},
	   	 success:function(data)
	   	 {
	   		 console.log("addActivityAdvanceEstimateRequest"+data);
    	  ...
    	  showMessage("successfully submitted the activity finance details of activity "+activityId);
    	  $("#activityMasterDetailsForm").trigger('reset');
   ...
   $('#ResourcePersonDetailsFormEdit').unbind().bind('submit',function(event)
	   		{
				...
				/* 	$.ajax({
						...
						"url" : "/resource/addActivityResource",
						"method" : "GET",
						 data : {"activityId":activityId,"resId":respid},
...
<div class="card card-info card-outline">
   <div class="card-header">
      <h3>Request For Academic Approval For Content And Relevance for Programes</h3>
   </div>
   <div class="card-body">
...
<!--             </div> -->
            ...
            <div class="form-group">
            <label for="brochure" class="col-sm-3 col-form-label"> Upload Information</label> 
            ...
            <div class="form-group">
               <label>Enter Date</label> <input tydespe="text" id="submitDate"
                  name="enterDate" class="form-control dp" disabled="disabled" required></input>
            </div>
            
             <div class="form-group" style="text-align: center">
               <button type="submit" class="btn btn-primary" id="mastersubmit">SUBMIT</button>
          ...
          <button type="button" id="centerActivity" class="btn btn-primary"
               data-toggle="modal" data-target="#dialogCenterActivityEdit" disabled="disabled">
...
<div class="modal fade" id="dialogCenterActivityEdit" tabindex="-1"
   role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
   <div class="modal-dialog modal-xl">
      <div class="modal-content">
         <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">Request For Administrative Sanction for Programmes
            </h5>
            <button type="button" class="close" data-dismiss="modal"
               aria-label="Close">
            <span aria-hidden="true">&times;</span>
         ...
         <div class="modal-body">
            <form id="formCenterActivityDetails"  enctype="multipart/form-data">
               <div class="form-group">
                  <div id="divGridTable2" class="row" >
                     <div class="col-md-12">
                              ...
                              <thead>
                                 <tr>
                                    ...
                                    <th scope="col">National Seminars Organised till date (
                                       Including this)
                                    <th scope="col">Extension Lectures/ similar programmes Organised
                                       till date ( Including this)
                                    </th>
                                    
                                    <th scope="col">Unassigned Grant Available
                                    </th>
                                 </tr>
                              </thead>
                              <tbody id="centerActivityeBody2" style="overflow-y:scroll">
                              </tbody>
                           </table>
                        </div>
                     </div>
               ...
               <div class="form-group row">
                  ...
                  there any external funding for the programme ? </label>
                  <div class="col-sm-6">
                     <select class="form-control" id="externalfundSelect" name="externalfundSelectname" required>
               ...
               <div class="form-group row">
                  ...
                  <div class="col-sm-6">
                     <select class="form-control" id="ugcSelect" name="ugcSelectname" required>
                        <option value="se">Select</option>
                        <option value="yes">Yes</option>
                        <option value="no">No</option>
                     </select>
               ...
               <div class="form-group row">
                  ...
                  <div class="col-sm-6">
                     <select class="form-control" id="privateFund" name="privateFundname" required>
                        <option value="se">Select</option>
                        <option value="yes">Yes</option>
                        <option value="no">No</option>
                     </select>
               ...
               <div class="form-group row">
                  ...
                  <div class="col-sm-6">
                     <select class="form-control" id="HonorMore3000" name="HonorMore3000" required>
                        <option value="se">Select</option>
                        <option value="yes">Yes</option>
                        <option value="no">No</option>
                     </select>
               ...
               <div id="expenditureGrid" class="row" style="display: none">
                  <div class="col-md-12">
                     <div class="table-responsive cell-border">
                        <table id="expeditureTable" class="table table-striped">
                           <thead>
                              <tr>
                 ...
                 <div class="form-group row"  class="col-sm-3"> <button type="button" id="totalExpenditure" class="btn btn-primary">Get Total Deviations And Estimate Expenditure</button></div>
                 <div class="form-group row">
                  <label for="activitytype" class="col-sm-2 col-form-label">Total NUALS Allocation Amount
                   ...
                   <div class="col-sm-2">
                     <input type="text" name="totalallocation" class="form-control"
                        id="totalallocationId" required value="0">
                  </div>
                  ...
                  <div class="col-sm-2">
                     <input type="text" name="total_estimated" class="form-control"
                        id="totalEstExpIdd" required value="0">
                  </div>
                  ...
                  <div class="col-sm-2">
                     <input type="text" name="totaldeviation" class="form-control"
                        id="totaldeviationId" required value="0">
                  </div>
               ...
               <div class="form-group row">
                  <label for="activitytype" class="col-sm-3 col-form-label">Total
                  Advance Required </label>
                  <div class="col-sm-6">
                     <input type="text" name="totalAdvanceReq" class="form-control"
                        id="totalAdvanceReqId" required value="0">
                  </div>
               </div>
				...
				<div class=" col-lg-6 col-md-6 col-sm-6 col-xs-6">
						 ...
						 <button type="button" id="activityFinance" class="btn btn-primary">Add</button>	
					</div>
					...
					<div class=" col-lg-6 col-md-6 col-sm-6 col-xs-6">
                       	<div class="form-group" style="text-align:left">
						   <button type="button" class="btn btn-secondary"
                        data-dismiss="modal" id="financePrint">PRINT</button>
					</div>
				</div>
				</div>
