		...
		success : function(data) {
			...
			onDataTableClick('BudgetFundTable', function() {
				 ...
				 var status=selectedRowFromDataTable[5];
					...
					$.ajax({
						"url" : "/expenditurehead/getBudgetFundAmount",
						"method" : "GET",
						"data":{"budheadId":budheadId,"finYear":finyear},
						
						success : function(data) {
							$('#bfverifiedRemarksId').val(data['bfverifiedRemarks']);
						},
						error : function(e) {
...
$('#budgetFundDetailsForm').unbind().bind('submit',function(event) 
{
			...
			var formCheck = "success";
				...
				$('#confId').unbind().bind('click',function(){
					...
					$.ajax({
						"url" : "/expenditurehead/approveBudgetFund",
						"method" : "POST",
						 data : formData,
						 success : function(data) {
                            showMessage(data);
    ...
    <div class="card-body">
	  <form id="budgetFundDetailsForm">
		...
		<div class="row">
			<div class=" col-lg-12 col-md-12 col-sm-12 col-xs-12">
		    <div class="form-group">
              	<label for="activitytype">Approve Remarks</label> <textarea id="bfapprovedRemarksId" name="bfapprovedRemarks" rows="4" cols="50"
                  class="form-control"></textarea>
            </div>
            </div>
            </div>	
						...
						<button type="submit" class="btn btn-primary" id="verifybtnid">AAPROVE</button>		
					</div>
				</div>
	  </form>
	</div>
