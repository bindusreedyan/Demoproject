...
function getAllActivityFinalOfficeApprovedByLogin() {
	console.log("loadAllActivityAcademicallyApproved");
	$.ajax({
		"url" : "/academicactivity/getAllActivityFinalOfficeApprovedByLogin",
		"method" : "GET",
...
function loadAllActivityForFinalApproval() {
	...
	$.ajax({
		"url" : "/academicactivity/getAllActivityFinancialApproved",
		"method" : "GET",
		success : function(data) {
...
$(document).ready(function()
	...
	getAllActivityFinalOfficeApprovedByLogin();
	$( "#tabs" ).tabs({
	   ...
	   //final approving and asno generation
	     $("#activityFinalApproval" ).on('click',function(event) {
	    	 var finalApprovalRemarks=$('#financialfinalApprovedRemarksId').val();
	              console.log("activityFinalApproval button clicked"+finalApprovalRemarks);
	              var userCode=$('#usercode').val();
	              console.log("userCode"+userCode);
		   	$.ajax({
		   				"url":"/academicactivity/activityFinalApprovalOffice",////
		   				"method":"GET",
		   				 data : {"activityId":activityId,"finalApprovalRemarks":finalApprovalRemarks},
		   				success:function(data)
		   				{
		   					if(data)
		   					{
	    					 ...
	    					 $.ajax({
	    				                 ...
	    				                 $('#advtab').append('<thead><tr><th scope="col">Head Of Expenditure</th><th scope="col">Advance Required</th><th scope="col">Briefly State Requirement with reason</th><th scope="col">As Granted Amount</th>');
	    									...
	    									// $('#financeofficeRemarkstd').append(financeOfficeApprovalRemarks);									 
	    									 $('#financialRemarkstd').append($('#financialApprovedRemarksId').val());	
	    									 $('#finalRemarkstd').append($('#financialfinalApprovedRemarksId').val());		
...
<div class="card card-info card-outline">
   <div class="card-header">
      <h3>Final Office Approval of Activity</h3>
   </div>
<div class="card-body">
<div class="row">
...
<!--                 <div class="form-group row"  class="col-sm-3"> <button type="button" id="totaladvancereq1" class="btn btn-primary">Get Total</button></div> -->
            ...
            <div class="form-group">
               <label for="remarks"> Remarks For Final Office Approval</label>
               <textarea id="financialfinalApprovedRemarksId" name="finalApprovedRemarks" rows="4" cols="50"
                  class="form-control"></textarea>
            </div>
            <div class="form-group" style="text-align: center">
               <input type="button" class="btn btn-primary" id="activityFinalApproval" value="Approve"></button>
            </div>
               </form>
             <div class="form-group" style="text-align: center">
...
<div  id="gridRow" class="card card-info card-outline">
  	<div class="card-header">
		  <h3>List Of Activities Finally Office Approved</h3>
	...
	<div class="col-md-12">
		<div class="table-responsive" id="reportDiv" style="display:none">
			<table class="table table-striped" id="repTable">
				...
				<tr td colspan="4"><center> FINAL OFFICE APPROVAL FOR PROGRAMMES CONDUCTED BY NUALS/ VARIOUS  CENTRES OF STUDY AND RESEARCH </center><td></tr>
				<tr><td colspan="4">
				...
				<tr><td  colspan="4"><u>Final office Approval Remarks </u></td></tr>   
